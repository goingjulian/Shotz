...
function _onConnection(websocket, req) {
  console.log(
    `INFO: New websocket connection with IP: ${req.connection.remoteAddress}, session: ${
      req.session.id
    } `
  );
...
export async function sendMessageQuizmaster(roomKey, message) {
  const quizmasterId = await GameDAO.getQuizmaster(roomKey);

  if (!quizmasterId) throw new ShotzException(`No quizmaster found for roomKey ${roomKey}`);
...
export async function sendMessageScoreBoards(roomKey, message) {
  const result = await GameDAO.getScoreBoards(roomKey);
  if (!result) {
